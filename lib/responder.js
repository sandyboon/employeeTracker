...
class Responder {
  ...
  async respondToUserChoice({ choices }) {
    switch (choices) {
      ...
      case questions.possibleActions.UPDATE_EMP_ROLE:
        ...
        console.log(colorCode.magenta, 'Employee Role Updated!');
        console.table(updatedEmp);
        break;

      case questions.possibleActions.UPDATE_EMP_MANAGER:
        let empUpdated = await this.updateEmployeeManager();
        console.log(colorCode.magenta, 'Employee Manager Updated!');
        console.table(empUpdated);
        break;
  ...
  async updateEmployeeRole() {
    const answers = await inquirer.prompt(
      questions.getEmployeeRoleUpdateQuestions(
        await this.employeeDb.getEmployeesWithRole(),
        await this.employeeDb.viewAllRoles()
      )
    );
  ...
  async updateEmployeeManager() {
    const answers = await inquirer.prompt(
      questions.getEmployeeManagerUpdateQuestions(
        await this.employeeDb.getEmployeesWithManager(),
        await this.employeeDb.getEmployeesWithRole()
      )
    );
    // console.log(answers);
    return await this.employeeDb.updateEmployeeManager(answers);
  }
